...
axios
	.get('https://api.cloudflare.com/client/v4/zones/' + process.env.CLOUDFLARE_ZONEIDENTIFIER + '/dns_records', {
		headers: {
		...
		axios
			...
			.then((response) => {
				...
				if (validIp.test(response.data.ip)) {
					...
					axios
						.put(
							'https://api.cloudflare.com/client/v4/zones/' + process.env.CLOUDFLARE_ZONEIDENTIFIER + '/dns_records/' + dnsRecord.id,
							JSON.stringify({
								type: dnsRecord.type,
								name: dnsRecord.name,
								content: currentIp,
